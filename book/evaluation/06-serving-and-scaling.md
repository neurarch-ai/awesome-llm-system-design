...
**More detail.** Two rows conceal a statistical subtlety. The average-masks-regression row is a base-rate effect: if a slice is 2 percent of traffic, even a total collapse on it moves the overall mean by only about 2 percent, which sits inside normal judge variance, so worst-slice gating is not optional but the only sensitive test. The canary-too-small row is bounded by statistical power: detecting a small effect at fixed significance and power needs a sample that grows roughly with the inverse square of the effect size, so halving the effect you want to catch roughly quadruples the required canary traffic and the time to a verdict.
