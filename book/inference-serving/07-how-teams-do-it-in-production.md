...
- **vLLM** [vLLM V1: a major upgrade to vLLM's core architecture](https://blog.vllm.ai/2025/01/27/v1-alpha-release.html) (Jan 2025): a ground-up rewrite of the scheduler, KV-cache manager, and worker that turns the optimizations above on by default. Its persistent-batch loop caches input tensors and applies only per-step diffs, cutting CPU overhead for about 1.7x higher throughput than V0. The lesson for 2025: the bottleneck at scale moved from the GPU kernels to CPU scheduling overhead, so the framework rewrite, not a new attention trick, was the win. *(deployment)*
