...
- **Salesforce** [Inside the Atlas Reasoning Engine](https://engineering.salesforce.com/inside-the-brain-of-agentforce-revealing-the-atlas-reasoning-engine/): model-agnostic reasoning and planning engine driving enterprise agent actions at scale; it grounds each request against a library of topics and actions, then plans and refines that plan before acting.
...
- **Anthropic** [Introducing the Model Context Protocol (MCP)](https://www.anthropic.com/news/model-context-protocol) (Nov 2024): the open standard that became the default way to connect agents to tools and data in 2025, replacing bespoke per-integration glue with one protocol for exposing tools, resources, and prompts; since adopted by OpenAI and Google. The 2025 interview signal is knowing that tool wiring is now a protocol (MCP), not a custom adapter per tool. *(product design)*
