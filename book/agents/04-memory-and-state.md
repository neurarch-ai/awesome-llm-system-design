...
**Provenance.** Long-term retrieval is RAG (Meta FAIR, 2020); the vector store behind it typically indexes with HNSW (Malkov and Yashunin, 2016) or IVF-PQ (FAISS by Meta).

**Tools.** The write, select, compress, isolate strategies map onto memory abstractions in LangChain and LangGraph and in LlamaIndex, which ship summary buffers, scratchpad stores, and per-step tool selection out of the box; Mem0 is a dedicated long-term agent-memory layer. Long-term retrieval is ordinary RAG, backed by a vector index such as FAISS (Meta), Qdrant, or Weaviate for the policy and history stores. Prefix caching is a provider feature (for example Anthropic prompt caching) where a stable prefix is paid for once and later reads are much cheaper than the initial write.

**Worked example.** An enterprise-RAG team building a support agent keeps the working transcript short-term because the model needs prior tool results to choose the next step, but it does not preload the customer's full order history. Instead it uses long-term retrieval, pulling only the policy sections and account facts relevant to the current ticket from a vector store, so the context does not bloat with history that could mislead the model. Verbose tool payloads go to an external scratchpad under the write strategy rather than being re-read every step, and once the transcript nears the context limit it compresses old history into a summary that preserves decisions while discarding raw API bodies. A token-heavy sub-task like parsing a long attached document runs in an isolated sub-context so the artifact never enters the main window, and prefix caching covers the stable system prompt and ticket across all steps.
