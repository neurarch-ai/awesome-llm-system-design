...
**Tools.** Mid-generation cancellation and continuous batching are provided by the inference engines themselves: vLLM, Hugging Face TGI, and SGLang all support aborting a request in flight and interleaving decode steps across streams for fair scheduling. The gateway that watches for disconnects, enforces the bounded per-stream buffer, and streams over SSE is typically an async server such as FastAPI or a Node service, fronted by a reverse proxy like NGINX or Envoy that can shed load with an HTTP 429 and route by priority. Horizontal scale-out is orchestrated by Kubernetes with a horizontal pod autoscaler keyed on in-flight streams rather than raw requests per second.

**Worked example.** A chat product treats cancel-on-disconnect with a bounded buffer as the non-negotiable baseline, since every orphaned tab otherwise pins an inference slot until the engine is told to abort. When decode outpaces a hopelessly slow consumer and the buffer fills, it aborts that stream rather than blocking the decode loop indefinitely and holding the slot hostage. During a traffic spike it first sheds excess load with a visible retry signal instead of silently queueing, then falls back to a smaller model so users see a quality dip rather than a blank screen. It routes Pro users through a priority queue ahead of free-tier traffic, and only reaches for horizontal scale-out when the overload is sustained and neither batching nor fallback can absorb it.
