...
**More detail.** Two rows have clean provenance and mechanism. The memory-wall fix pairs ZeRO (Microsoft) optimizer, gradient, and parameter sharding with its native FSDP (Meta) realization; the cost is that ZeRO-3 all-gathers each layer's parameters just before its forward and backward pass, which is the extra communication that surfaces as lower MFU. The compute-misallocation fix is Chinchilla (DeepMind, 2022) sizing at roughly 20 tokens per parameter, but that minimizes only training compute: when the model will serve billions of tokens, deliberately overtraining a smaller model past the Chinchilla point trades one-time training compute for a permanently cheaper per-token inference footprint.
