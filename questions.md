...
|---|---|---|
| Make our LLM feature cheaper without tanking quality | Right-sizing and routing, cascades, semantic caching, prompt compression, quantization | [11 cost and routing](topics/11-cost-optimization-and-model-routing.md), [04 inference serving](topics/04-inference-serving-at-scale.md) |
| How do you know a live LLM app still works, and catch a hallucination spike? | Online eval without ground truth, LLM-as-judge, grounding checks, regression detection | [12 production monitoring](topics/12-production-monitoring-and-observability.md), [06 evaluation](topics/06-evaluation-system.md) |
...
| We are choosing between three models. Benchmark them and tell us which to build on | Harness protocol, contamination control, paired statistics, cost-matched comparison, "not distinguishable" as a verdict | [benchmarking a model](book/benchmark-eval/), [06 eval](topics/06-evaluation-system.md) |
| Our benchmark number does not match the vendor's. Explain the gap | Prompt template, scoring mode, token budget, parser, decode settings, benchmark version | [benchmarking a model](book/benchmark-eval/03-the-harness.md) |
| Make this model fit on one GPU without hurting latency | Where the bytes and microseconds go, weight vs KV quantization, kernel support, mixed precision by layer | [model compression](book/model-compression/), [02 KV cache](topics/02-long-context-and-kv-cache.md), [04 serving](topics/04-inference-serving-at-scale.md) |
| Ship an assistant that runs on-device | Memory ceiling, NPU formats, distilling a small student, adapters over one resident base, server handoff | [model compression](book/model-compression/06-serving-and-scaling.md), [05 post-training](topics/05-post-training-pipeline.md) |
| Our product uses a reasoning model and p99 latency is unpredictable | Thinking-token budgets, output-length variance, effort routing, verifier economics, when not to use one | [reasoning and test-time compute](book/reasoning-serving/), [04 serving](topics/04-inference-serving-at-scale.md), [11 cost](topics/11-cost-optimization-and-model-routing.md) |
| Guarantee that our tool calls and JSON outputs are always valid | Constrained decoding, schema design, retry and repair, validity as a measured rate | [agents](book/agents/), [rag-serving](book/rag-serving/05-generation-and-grounding.md) |
...
| Design an image generation product, then make it cheaper | Denoiser evaluations as the cost unit, guidance and step count, step distillation, compiled engines and cold start, safety and provenance | [19 generation serving](topics/19-generation-serving.md), [11 cost](topics/11-cost-optimization-and-model-routing.md) |
| Add video generation to our product | Frames as an extra cost axis, temporal consistency, chunked generation, why it is a queued job | [19 generation serving](topics/19-generation-serving.md), [04 serving](topics/04-inference-serving-at-scale.md) |
