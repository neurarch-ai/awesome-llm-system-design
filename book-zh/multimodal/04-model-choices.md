...
一个做过指令微调的 VLM 仍然会描述图里根本没有的物体。原因是结构性的，不是偶然：
监督微调教的是"好答案长什么样"，而一个关于合理物体的流畅回答，看起来就很好。只做
纯文本的偏好微调也修不了，因为那批偏好数据从来不需要对着一张图核对。

三种有视觉依据的偏好数据，按成本从低到高：

| 做法 | 负样本从哪来 | 成本 | 注意 |
|---|---|---|---|
| 生成负样本（[POVID](https://arxiv.org/abs/2402.11411)） | 破坏图像，或往一个好答案里注入幻觉细节，把结果当作不被偏好的那个 | 最便宜，不需要人标 | 负样本是你自己造的，模型学的是你的破坏方式，不是人的口味 |
| 事实增强的 RLHF（[LLaVA-RLHF](https://arxiv.org/abs/2309.14525)） | 人来比较，但奖励模型额外拿到 caption 和参考事实，才分得清"有依据"和"流畅" | 中等 | 奖励模型必须拿到这些额外依据，否则它又去奖励流畅度 |
| 细粒度纠错反馈（[RLHF-V](https://arxiv.org/abs/2312.00849)） | 人直接改掉出现幻觉的那一段，而不是给整条回答排序 | 单条最贵，单位效果最便宜 | 段落级标注需要真正的标注工具和标注规范 |

这里的设计教训不止适用于幻觉：**一个偏好信号的"有依据"程度，取决于标注者或生成器
被强制看了什么。** 没有对着图收集的排序，是一个披着多模态标签的文本质量信号。

**要和通用质量分开测。** 物体幻觉有自己的指标，POPE 和 CHAIR，在
[评估](05-evaluation.md)那一节。如果一次偏好训练把胜率提上去了而 CHAIR 变差，那就是
拿依据换了流畅度，正是这一节要防的失败，而它在聚合分数上完全看不见。

**它在整个栈里的位置。** 对齐是最后一个杠杆，不是第一个。更好的 grounding 数据、
把模型产生幻觉的那部分细节用更高分辨率或切块喂进去、以及解码时的约束，都更便宜。
只有当模型看得见证据却还是说别的，才轮到偏好微调。
